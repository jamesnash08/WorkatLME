...
function checkParams(results, payload, cb) {
    ...
    if (results.paymentTerms == undefined) {
        ...
        if (results.paymentTerms == 'T') {
            ...
            payload.Error = "Please provide a Valid Payment Type!";
        }
    ...
    if (results.shipmentDetails == undefined) {
        payload.Error = "Please provide Commodities!";
        //fail();
        //break;
    } else {

        commodities = results.shipmentDetails.CommodityLine;

        console.log("Commodities: " + commodities.length);

        for (var idx = 0; idx <= commodities.length; idx++) {

            if (commodities[idx].consignedName == undefined) {

            } else if (commodities[idx].destZipCode == undefined) {

                if (commodities[idx].destZipCode.length > 2) {
                    payload.Error = "Please provide Destination Zip Code for commodity: " + idx;
                }

            } else if (commodities[idx].destCity == undefined) {

                if (commodities[idx].destCity.length > 2) {
                    payload.Error = "Please provide Destination City for commodity: " + idx;
                }

            } else if (commodities[idx].destState == undefined) {

                if (commodities[idx].destState.length > 2) {
                    payload.Error = "Please provide Destination State for commodity: " + idx;
                }

            } else if (commodities[idx].loadNumber == undefined) {

                if (commodities[idx].loadNumber.length > 2) {
                    payload.Error = "Please provide Destination Load Number for commodity: " + idx;
                }

            } else if (commodities[idx].units == undefined) {

                if (commodities[idx].units.length > 2) {
                    payload.Error = "Please provide Destination Units for commodity: " + idx;
                }

            } else if (commodities[idx].unitType == undefined) {

                if (commodities[idx].unitType.length > 2) {
                    payload.Error = "Please provide Destination Unit Type for commodity: " + idx;
                }

            } else if (commodities[idx].weight == undefined) {

                if (commodities[idx].weight.length > 2) {
                    payload.Error = "Please provide Destination Weight for commodity: " + idx;
                }

            } else if (commodities[idx].freezable == undefined) {

                if (commodities[idx].freezable.length > 2) {
                    payload.Error = "Please provide Destination Freezable for commodity: " + idx;
                }

            } else if (commodities[idx].hazMat == undefined) {

                if (commodities[idx].hazMat.length > 2) {
                    payload.Error = "Please provide Destination hazMat for commodity: " + idx;
                }

            } else if (commodities[idx].xgold == undefined) {

                if (commodities[idx].xgold.length > 2) {
                    payload.Error = "Please provide Destination XGold for commodity: " + idx;
                }
...
module.exports = {
    ...
    callCSM712: callCSM712,
    getUserAccount: getUserAccount,
    updatePartnerLog: updatePartnerLog,
    checkServiceDay: checkServiceDay,
    nextServiceDay: nextServiceDay,
    formatDate: formatDate,
    callCSM751: callCSM751,
    checkNonService: checkNonService,
    callCSM714: callCSM714,
    callCSM721: callCSM721,
    callEDM310: callEDM310,
    callECC301CL: callECC301CL,
