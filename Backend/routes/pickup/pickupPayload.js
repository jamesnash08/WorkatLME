function setupPayload(data) {
    ...
    if (data.quoteNumber != undefined) {
        ...
        payload["Quote"] = {
            ...
            totalcharge: "",
            showStd: "",
            numHandlingUnits: data.numHandlingUnits,
            xgoldmsg: ""
        }
        ...
        if (!data.commLines && data.commClass1) {
            data.commLines = [];
            for (var i = 1; i <= 10; i++) {

                if (data["commClass" + i] && data["commweight" + 1])
                    data.commLines.push({
                        commClass: data["commClass" + i],
                        commweight: Number(data["commweight" + i]).toFixed(0)
                    });
                else if (data["commClass" + i] && !data["commweight" + i]) {
        ...
        if (data.prvtRsdncDel == "Y")
            addQuoteAccs(payload, "HDP", "");
        if (data.prvtRsdncPckup == "Y")
